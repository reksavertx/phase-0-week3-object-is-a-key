...
function countProfit(shoppers) {
  ...
  for (let i = 0 ; i < listBarang.length; i++) {
    ...
    for (let q = 0; q < shoppers.length; q++) {
        if (shoppers[q]['product'] === listBarang[i][0]) {
            pembeli.push(shoppers[q]['name']);

            if (shoppers[q]['amount'] > stok) {
                continue
            }

            stok-=shoppers[q]['amount'];
        }
    }
    hasil.push({
        ...
        "shoppers" : pembeli,
        "leftover" : stok,
        "totalprofit" : (listBarang[i][2] - stok) * listBarang[i][1]
    });
  }
  return hasil;
}
